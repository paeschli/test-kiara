...
static char *user_headername = NULL;
static void handle_H_option(char *z){
  user_headername = (char *) malloc( lemonStrlen(z)+1 );
  if( user_headername==0 ){
    memory_error();
  }
  strcpy(user_headername, z);
}
...
int main(int argc, char **argv)
{
  ...
  static struct s_options options[] = {
    ...
    {OPT_FSTR, "O", (char*)handle_O_option, "Output generated code to a different file."},
    {OPT_FSTR, "H", (char*)handle_H_option, "Output generated header to a different file."},
...
PRIVATE FILE *file_open_ex(
  ...
  if( lemp->outname ) free(lemp->outname);
  lemp->outname = (char*)malloc(lemonStrlen(filename)+1);
  if( lemp->outname==0 ){
    fprintf(stderr,"Can't allocate space for a filename.\n");
    exit(1);
  }
  strcpy(lemp->outname, filename);
...
void ReportTable(
  ...
  if (user_outputname != 0)
    out = file_open_ex(lemp, user_outputname, "wb");
  else
...
void ReportHeader(struct lemon *lemp)
{
  ...
  if (user_headername != 0)
    in = file_open_ex(lemp, user_headername, "rb");
  else
    in = file_open(lemp,".h","rb");
  ...
  if (user_headername != 0)
    out = file_open_ex(lemp, user_headername, "wb");
  else
