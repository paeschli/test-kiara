...
from win32con import (HANDLE_FLAG_INHERIT, STARTF_USESTDHANDLES,
                      STARTF_USESHOWWINDOW, CREATE_NEW_CONSOLE, SW_SHOW, SW_HIDE,
                      ...
                      OPEN_EXISTING, PROCESS_ALL_ACCESS, MAXIMUM_ALLOWED, STILL_ACTIVE)
...
class winspawn(spawn):
    ...
    def _spawn(self, command, args=None):
        ...
        pycmd = 'import sys; sys.path = %r + sys.path; import winpexpect; winpexpect._stub(r"%s", r"%s", r"%s", r"%s")' \
                    % (spath, cmd_name, stdin_name, stdout_name, stderr_name)
    ...
    def isalive(self):
        ...
        if self.exitstatus is not None:
            return False
        exitstatus = GetExitCodeProcess(self.child_handle)
